...
with open(os.path.expandvars(external_files + version_name), 'r') as version_file:
    ...
    if version_web != version:
        ...
        os.startfile(os.path.expandvars('%APPDATA%\\Neko\\NekoInstaller.exe'))
        exit()
    else:
...
def on_closing():
    ...
    with open(os.path.expandvars(external_files + 'likes'), 'w') as likes_file:
...
def _from_rgb(rgb):
    ...
    str = "#%02x%02x%02x" % rgb
    return str
def create_image(img, mode):
    global ui

    if mode is False:
        canvas_size = window_size[0], window_size[1]
        img_fg = resize_image(img, screen_size, True)
        img_bg = resize_image(img_fg, screen_size, False)
    else:
        canvas_size = window_size[0]-ui, window_size[1]
        img_fg = resize_image(img, canvas_size, True)
        img_bg = resize_image(img_fg, canvas_size, False)
...
def update(window_size):
    ...
    img.convert('RGB')
    final_image, pos = create_image(img, False)
...
def set_background():
    ...
    final_image.save(os.path.expandvars(external_files + background_name))

    folder = os.path.expandvars(external_files + background_name)
